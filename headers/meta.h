...
#define MOMO_COMPILE_CHECK(expr, msg) compile_checker<expr> CONCAT(ERROR_, msg); (void)CONCAT(ERROR_, msg);

	template<typename To, typename From>
	To safe_reinterpret_cast(From from)
	{
		MOMO_COMPILE_CHECK(sizeof(To) <= sizeof(From), cannot_convert_provided_types);
		return reinterpret_cast<To>(from);
	}

	class type_info
	{
		const ::std::type_info* inner = nullptr;
	public:
		type_info() = default;
		type_info(const ::std::type_info& type);
		bool operator==(const type_info& type) const;
		bool operator!=(const type_info& type) const;
		const char* name() const;
		bool before(const type_info& type) const;
	};

	std::ostream& operator<<(std::ostream& out, const type_info& type);

	template<typename T>
	type_info typeof(const T& value)
	{
		return type_info(typeid(value));
	}
	...
	namespace typelist
	{
		...
		using WidgetInfo = GenScatterHierarchy<TypeList(int, std::string, float), Holder>;
		*/
	}
}